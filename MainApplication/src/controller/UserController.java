        ...
        if (request.getParameter("submit") != null && "user".equals(request.getParameter("controller"))) {
            switch (request.getParameter("action")) {
                ...
                case "weather": {
                    ...
                    String page = "http://battuta.medunes.net/api/region/" + user.getCountryCode() + "/all/?key=7578fefe26854e880722d16cb2a9f344";
                    String json = getFromUrl(page);
                    JsonArray regions = JsonParser.parseString(json).getAsJsonArray();
                    for (JsonElement region : regions) {
                        String page2 = "https://geo-battuta.net/api/city/" + user.getCountryCode() + "/search/?region=" + region.getAsJsonObject().get("region").getAsString().replace(" ", "+") + "&key=7578fefe26854e880722d16cb2a9f344";
